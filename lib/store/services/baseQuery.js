...
export const baseQueryWithReauth = async (args, api, extraOptions) => {
  ...
  // Refresh before toasting so expired sessions don't flash an error
  ...
  // getProfile 401 is expected when logged out — don't toast
  const skipToast =
    result.error?.status === 401 && api.endpoint === 'getProfile';
