...
export const authApi = api.injectEndpoints({
  ...
  query: (body) => ({
    url: "/api/v1/user/auth/chackout/",
    method: "POST",
    body,
  }),
}),

    flittPaymentSuccess: builder.query({
      query: (orderId) => ({
        url: `/api/v1/user/auth/payments/flitt/success/?order_id=${orderId}`,
        method: "GET",
      }),
      providesTags: ["User"],
    }),

    flittPaymentCancel: builder.query({
      query: (orderId) => ({
        url: `/api/v1/user/auth/payments/flitt/cancel/?order_id=${orderId}`,
        method: "GET",
      }),
    }),
...
export const {
  ...
  useFlittPaymentSuccessQuery,
  useLazyFlittPaymentSuccessQuery,
  useFlittPaymentCancelQuery,
  useLazyFlittPaymentCancelQuery,
} = authApi;